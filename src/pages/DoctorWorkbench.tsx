...
import { allSpecialties } from "@/lib/caseClassification";
...
type SpecialtyFilter = "all" | "fallback" | string;
...
export default function DoctorWorkbench() {
  ...
  const [showOtherRare, setShowOtherRare] = useState(false); // For rare/other diseases
  ...
  const loadCases = async () => {
    try {
      ...
      // Fetch ALL cases (both medical and wellness) - doctors can see everything
      // They can filter by specialty, category, etc.
      ...
      // Show ALL cases to doctors - they can filter as needed
      setCases(data || []);
    } catch (error) {
  ...
  const filteredCases = cases
    ...
    .filter(c => {
      ...
      // Other/rare diseases filter (cases without specialty or with "Other" specialty)
      // This includes ALL cases that haven't been assigned a specialty yet
      ...
      if (selectedSpecialties.length > 0) {
        // If case has no assigned specialties, include it when "Other/Rare" isn't active
        // (these are unclassified cases that need attention)
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 pb-2 border-b">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <Label htmlFor="fallback" className="text-sm flex items-center gap-1 cursor-pointer">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <Label htmlFor="other-rare" className="text-sm cursor-pointer">
                          Rare / Other Diseases
                        </Label>
                      </div>
                    </div>
                    
                    {/* All specialties checkboxes */}
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                      {allSpecialties.filter(s => s !== 'Other').map(specialty => (
